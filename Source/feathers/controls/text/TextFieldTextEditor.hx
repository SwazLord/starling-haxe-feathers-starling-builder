...
class TextFieldTextEditor extends BaseTextEditor implements ITextEditor implements INativeFocusOwner {
	...
	**/
	...
	private function get_currentTextFormat():openfl.text.TextFormat {
	...
	**/
	...
	private function get_textFormat():openfl.text.TextFormat {
	...
	private function get_disabledTextFormat():openfl.text.TextFormat {
	...
	private function get_isSelectable():Bool {
	...
	private function get_antiAliasType():String {
	...
	private function get_gridFitType():String {
	...
	private function get_useGutter():Bool {
	...
	private function get_setTouchFocusOnEndedPhase():Bool {
		return false;
	}
	...
	**/
	...
	private function get_updateSnapshotOnScaleChange():Bool {
	...
	private function get_useSnapshotDelayWorkaround():Bool {
	...
	private function get_resetScrollOnFocusOut():Bool {
	...
	**/
	override public function render(painter:Painter):Void {
		if (this.textSnapshot != null && this._updateSnapshotOnScaleChange) {
			...
			if (GeomUtils.matrixToScaleX(matrix) != this._lastGlobalScaleX || GeomUtils.matrixToScaleY(matrix) != this._lastGlobalScaleY) {
	...
	private function commitStylesAndData(textField:TextField):Void {
		...
		if (!this._embedFonts && this._currentTextFormat == this._fontStylesTextFormat) {
			...
			textField.embedFonts = SystemUtil.isEmbeddedFont(this._currentTextFormat.font, this._currentTextFormat.bold, this._currentTextFormat.italic,
				FontType.EMBEDDED);
		} else {
		...
		this._needsNewTexture = this._needsNewTexture
			...
			|| (textureRoot != null
				&& (textureRoot.scale != starling.contentScaleFactor
					|| this._snapshotWidth != textureRoot.nativeWidth
					|| this._snapshotHeight != textureRoot.nativeHeight));
	}
