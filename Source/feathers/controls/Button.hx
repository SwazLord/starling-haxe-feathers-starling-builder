...
class Button extends BasicButton implements IFocusDisplayObject implements ITextBaselineControl {
	...
	override public function render(painter:Painter):Void {
		...
		if (scale != 1) {
			...
			matrix.translate(Math.fround((1 - scale) / 2 * this.actualWidth), Math.fround((1 - scale) / 2 * this.actualHeight));
	...
	override private function autoSizeIfNeeded():Bool {
		...
		if (needsWidth) {
			...
			if (this.currentSkin != null && this.currentSkin.width > newWidth) {
		...
		if (needsHeight) {
			...
			if (this.currentSkin != null && this.currentSkin.height > newHeight) {
	...
	private function refreshLabelTextRendererDimensions(forMeasurement:Bool):Void {
		...
		if (this.currentIcon != null) {
			...
			if (this._iconPosition == RelativePosition.LEFT
				|| this._iconPosition == RelativePosition.LEFT_BASELINE
				|| this._iconPosition == RelativePosition.RIGHT
				|| this._iconPosition == RelativePosition.RIGHT_BASELINE) {
