...
class ImageLoader extends FeathersControl {
	 ...
	 * @private
	 */
	public function set_textureCache(value:TextureCache):TextureCache {
	...
	public function set_loadingTexture(value:Texture):Texture {
		if (this.processStyleRestriction(this.set_loadingTexture)) {
	...
	public function set_errorTexture(value:Texture):Texture {
		if (this.processStyleRestriction(this.set_errorTexture)) {
	...
	public function get_scaleFactor():Float {
	...
	public function set_textureSmoothing(value:String):String {
		if (this.processStyleRestriction(this.set_textureSmoothing)) {
			return get_textureSmoothing();
		}
	...
	public function get_tileGrid():Rectangle {
	 ...
	 * @private
	 */
	public function set_tileGrid(value:Rectangle):Rectangle {
	...
	public function set_color(value:UInt):UInt {
		if (this.processStyleRestriction(this.set_color)) {
			return get_color();
		}
	...
	public function set_scaleContent(value:Bool):Bool {
		if (this.processStyleRestriction(this.set_scaleContent)) {
	...
	public function set_maintainAspectRatio(value:Bool):Bool {
		if (this.processStyleRestriction(this.set_maintainAspectRatio)) {
	...
	public function set_scaleMode(value:String):String {
		if (this.processStyleRestriction(this.set_scaleMode)) {
	...
	public function set_textureQueueDuration(value:Float):Float {
		...
		if (this._delayTextureCreation) {
			if ((this._pendingBitmapDataTexture != null || this._pendingRawTextureData != null)
				&& oldDuration == Math.POSITIVE_INFINITY
				&& this._textureQueueDuration < Math.POSITIVE_INFINITY) {
	...
	public function get_asyncTextureUpload():Bool {
	 ...
	 * @private
	 */
	public function set_asyncTextureUpload(value:Bool):Bool {
	...
	public function get_loaderContext():LoaderContext {
	 ...
	 * @private
	 */
	public function set_loaderContext(value:LoaderContext):LoaderContext {
	...
	private function commitData():Void {
		if (Std.isOfType(this._source, Texture)) {
			...
			var sourceURL:String = cast(this._source, String);
			if (sourceURL == null) {
				this._lastURL = null;
	...
	private function layout():Void {
		...
		if ((!this._scaleContent || (this._maintainAspectRatio && this._scaleMode != ScaleMode.SHOW_ALL))
			...
			var mask:Quad = cast(this.image.mask, Quad);
			if (mask != null) {
				mask.x = 0;
				mask.y = 0;
			...
			mask = cast(this.image.mask, Quad);
			if (mask != null) {
	...
	private function findSourceInCache():Bool {
		var cacheKey:String = this.sourceToTextureCacheKey(this._source);
		if (this._textureCache != null && !this._isRestoringTexture && cacheKey != null && this._textureCache.hasTexture(cacheKey)) {
	...
	private function replaceBitmapDataTexture(bitmapData:BitmapData):Void {
		...
		if (this._texture == null) {
			...
			try {
				this._texture = Texture.empty(bitmapData.width / this._scaleFactor, bitmapData.height / this._scaleFactor, true, false, false,
					this._scaleFactor, this._textureFormat);
			} catch (error:Error) {
				this.cleanupTexture();
				this.invalidate(FeathersControl.INVALIDATION_FLAG_DATA);
				this.dispatchEventWith(Event.IO_ERROR, false, new IOErrorEvent(IOErrorEvent.IO_ERROR, false, false, error.toString()));
				return;
			}
		...
		if (this._asyncTextureUpload) {
			this._texture.root.uploadBitmapData(bitmapData, function(_) {
				if (image != null) {
	 ...
	 * @private
	 */
	private function createTextureOnRestore(texture:Texture, source:Dynamic, format:String, scaleFactor:Float):ASFunction {
		return function(_) {
			if (_texture == texture) {
				texture_onRestore();
				return;
	...
	private function onRestore_onComplete(event:starling.events.Event):Void {
		var otherLoader:ImageLoader = cast(event.currentTarget, ImageLoader);
	...
	private function loader_completeHandler(event:flash.events.Event):Void {
		...
		var canReuseTexture:Bool = this._texture != null
	 ...
	 * @private
	 */
	private function orphanedLoader_completeHandler(event:flash.events.Event):Void {
		var loaderInfo:LoaderInfo = cast(event.currentTarget, LoaderInfo);
	 ...
	 * @private
	 */
	private function orphanedLoader_errorHandler(event:flash.events.Event):Void {
		var loaderInfo:LoaderInfo = cast(event.currentTarget, LoaderInfo);
		loaderInfo.removeEventListener(flash.events.Event.COMPLETE, orphanedLoader_completeHandler);
		loaderInfo.removeEventListener(IOErrorEvent.IO_ERROR, orphanedLoader_errorHandler);
		loaderInfo.removeEventListener(SecurityErrorEvent.SECURITY_ERROR, orphanedLoader_errorHandler);
