 ...
 */
class VerticalLayout extends BaseLinearLayout implements IVariableVirtualLayout implements ITrimmedVirtualLayout implements IGroupedLayout
		implements IDragDropLayout {
	/**
	 * Constructor.
	 */
	public function new() {
	...
	public function get_stickyHeader():Bool {
		return this._stickyHeader;
	}

	public function set_stickyHeader(value:Bool):Bool {
	...
	public function get_headerIndices():Array<Int> {
		return this._headerIndices;
	}

	public function set_headerIndices(value:Array<Int>):Array<Int> {
	...
	public function get_maxRowCount():Int {
		return this._maxRowCount;
	}

	public function set_maxRowCount(value:Int):Int {
		if (value < 0) {
	...
	public function get_headerScrollPositionVerticalAlign():String {
		return this._headerScrollPositionVerticalAlign;
	}

	public function set_headerScrollPositionVerticalAlign(value:String):String {
	...
	override function get_requiresLayoutOnScroll():Bool {
		return this._useVirtualLayout || (this._headerIndices != null && this._stickyHeader); // the header needs to stick!
	}
	...
	public function layout(items:Array<DisplayObject>, viewPortBounds:ViewPortBounds = null, result:LayoutBoundsResult = null):LayoutBoundsResult {
		...
		if (!this._useVirtualLayout
			|| this._hasVariableItemDimensions
			|| this._distributeHeights
			...
			|| needsExplicitWidth) // isNaN
		{
			...
			this.validateItems(items, explicitWidth
				- this._paddingLeft
				- this._paddingRight, minWidth
				- this._paddingLeft
				- this._paddingRight,
				maxWidth
				- this._paddingLeft
		...
		totalWidth = maxItemWidth + this._paddingLeft + this._paddingRight;
		...
		if (availableHeight != availableHeight) // isNaN
		{
			...
			if (this._requestedRowCount > 0) {
				...
				if (this._maxRowCount > 0) {
					if (this._useVirtualLayout) {
						maxRowAvailableHeight = this._maxRowCount * (calculatedTypicalItemHeight + this._gap)
							- this._gap
							+ this._paddingTop
							+ this._paddingBottom;
					}
					...
					case HorizontalAlign.CENTER:
						{
							...
							item.x = pivotX
								+ boundsX
								+ this._paddingLeft
								+ Math.round((horizontalAlignWidth - this._paddingLeft - this._paddingRight - item.width) / 2);
							break;
						}
					default: // left
						{
	 ...
	 * @private
	 */
	public function positionDropIndicator(dropIndicator:DisplayObject, index:Int, x:Float, y:Float, items:Array<DisplayObject>, width:Float,
			height:Float):Void {
		var indexOffset:Int = 0;
		var itemCount:Int = items.length;
		var totalItemCount:Int = itemCount;
	 ...
	 * @private
	 */
	private function validateItems(items:Array<DisplayObject>, explicitWidth:Float, minWidth:Float, maxWidth:Float, explicitHeight:Float, minHeight:Float,
			maxHeight:Float, distributedHeight:Float):Void {
		var needsHeight:Bool = explicitHeight != explicitHeight; // isNaN
	...
	private function prepareTypicalItem(justifyWidth:Float):Void {
		...
		if (this._horizontalAlign == HorizontalAlign.JUSTIFY && justifyWidth == justifyWidth) // !isNaN
		{
			hasSetWidth = true;
