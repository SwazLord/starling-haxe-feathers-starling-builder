...
class HorizontalLayout extends BaseLinearLayout implements IVariableVirtualLayout implements ITrimmedVirtualLayout implements IDragDropLayout {
	...
	public function layout(items:Array<DisplayObject>, viewPortBounds:ViewPortBounds = null, result:LayoutBoundsResult = null):LayoutBoundsResult {
			...
			|| needsExplicitHeight) // isNaN
		{
			...
			this.validateItems(items, explicitHeight
				- this._paddingTop
				- this._paddingBottom, minHeight
				- this._paddingTop
				- this._paddingBottom,
				maxHeight
				- this._paddingTop
				- this._paddingBottom, explicitWidth
				- this._paddingLeft
				- this._paddingRight,
				minWidth
				- this._paddingLeft
				- this._paddingRight, maxWidth
				- this._paddingLeft
		...
		for (i in 0...itemCount) {
			...
			if (this._useVirtualLayout && item == null) {
				...
				if (!this._hasVariableItemDimensions || cachedWidth != cachedWidth) // isNaN
				{
				...
				if (this._useVirtualLayout) {
					if (this._hasVariableItemDimensions) {
						if (itemWidth != cachedWidth) {
							...
							// this item resizes.
							if (positionX < scrollX && cachedWidth != cachedWidth && // isNaN
		...
		if (availableWidth != availableWidth) // isNaN
		{
			if (this._requestedColumnCount > 0) {
				...
				if (this._maxColumnCount > 0) {
					if (this._useVirtualLayout) {
						maxColumnAvailableWidth = (calculatedTypicalItemWidth + this._gap) * this._maxColumnCount - this._gap + this._paddingLeft
							+ this._paddingRight;
					}
	...
	public function measureViewPort(itemCount:Int, viewPortBounds:ViewPortBounds = null, result:Point = null):Point {
		...
		if (needsWidth) {
			...
			if (this._requestedColumnCount > 0) {
				...
				if (this._maxColumnCount > 0) {
					var maxColumnResultWidth:Float = (calculatedTypicalItemWidth + this._gap) * this._maxColumnCount - this._gap + this._paddingLeft
						+ this._paddingRight;
	 ...
	 * @private
	 */
	private function validateItems(items:Array<DisplayObject>, explicitHeight:Float, minHeight:Float, maxHeight:Float, explicitWidth:Float, minWidth:Float,
			maxWidth:Float, distributedWidth:Float):Void {
		var needsWidth:Bool = explicitWidth != explicitWidth; // isNaN
		var needsHeight:Bool = explicitHeight != explicitHeight; // isNaN
	...
	private function prepareTypicalItem(justifyHeight:Float):Void {
		...
		if (this._verticalAlign == VerticalAlign.JUSTIFY && justifyHeight == justifyHeight) // !isNaN
		{
			hasSetHeight = true;
