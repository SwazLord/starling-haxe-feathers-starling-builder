...
class AnchorLayout extends EventDispatcher implements ILayout {
	 ...
	 * @inheritDoc
	 */
	public function calculateNavigationDestination(items:Array<DisplayObject>, index:Int, keyCode:UInt, bounds:LayoutBoundsResult):Int {
		return index;
	}
	 ...
	 * @inheritDoc
	 */
	public function getNearestScrollPositionForIndex(index:Int, scrollX:Float, scrollY:Float, items:Array<DisplayObject>, x:Float, y:Float, width:Float,
	 ...
	 * @private
	 */
	private function measureViewPort(items:Array<DisplayObject>, viewPortWidth:Float, viewPortHeight:Float, result:Point):Point {
	 ...
	 * @private
	 */
	private function layoutVector(items:Array<DisplayObject>, unpositionedItems:Array<DisplayObject>, boundsX:Float, boundsY:Float, viewPortWidth:Float,
		...
		for (i in 0...itemCount) {
			var item:DisplayObject = items[i];
			var layoutItem:ILayoutDisplayObject = SafeCast.safe_cast(item, ILayoutDisplayObject);
			if (layoutItem == null || !layoutItem.includeInLayout) {
				continue;
			}
			var layoutData:AnchorLayoutData = SafeCast.safe_cast(layoutItem.layoutData, AnchorLayoutData);
			if (layoutData == null) {
				continue;
			}
	 ...
	 * @private
	 */
	private function positionHorizontally(item:ILayoutDisplayObject, layoutData:AnchorLayoutData, boundsX:Float, boundsY:Float, viewPortWidth:Float,
			viewPortHeight:Float):Void {
		var uiItem:IMeasureDisplayObject = cast(item, IMeasureDisplayObject);
		...
		if (percentWidth == percentWidth) // !isNaN
		{
			...
			if (uiItem.explicitMaxWidth == uiItem.explicitMaxWidth && uiItem.explicitMaxWidth < itemWidth) {
		...
		var right:Float = layoutData.right;
		var hasRightPosition:Bool = right == right; // !isNaN
		var rightAnchorDisplayObject:DisplayObject = null;
		var horizontalCenterAnchorDisplayObject:DisplayObject = null;
		var xPositionOfCenter:Float;
		if (hasRightPosition) {
			rightAnchorDisplayObject = layoutData.rightAnchorDisplayObject;
			if (hasLeftPosition) {
				...
				if (horizontalCenterAnchorDisplayObject != null) {
					xPositionOfCenter = horizontalCenterAnchorDisplayObject.x
						- horizontalCenterAnchorDisplayObject.pivotX
						+ Math.round(horizontalCenterAnchorDisplayObject.width / 2)
						+ horizontalCenter;
				} else {
	 ...
	 * @private
	 */
	private function positionVertically(item:ILayoutDisplayObject, layoutData:AnchorLayoutData, boundsX:Float, boundsY:Float, viewPortWidth:Float,
			viewPortHeight:Float):Void {
		var uiItem:IMeasureDisplayObject = cast(item, IMeasureDisplayObject);
		var percentHeight:Float = layoutData.percentHeight;
		...
		var bottom:Float = layoutData.bottom;
		var hasBottomPosition:Bool = bottom == bottom; // !isNaN
		var bottomAnchorDisplayObject:DisplayObject = null;
		var verticalCenterAnchorDisplayObject:DisplayObject = null;
		var yPositionOfCenter:Float;
		if (hasBottomPosition) {
			bottomAnchorDisplayObject = layoutData.bottomAnchorDisplayObject;
			if (hasTopPosition) {
				...
				if (verticalCenterAnchorDisplayObject != null) {
					yPositionOfCenter = verticalCenterAnchorDisplayObject.y - verticalCenterAnchorDisplayObject.pivotY
						+ Math.round(verticalCenterAnchorDisplayObject.height / 2) + verticalCenter;
				} else {
	...
	private function isReadyForLayout(layoutData:AnchorLayoutData, index:Int, items:Array<DisplayObject>, unpositionedItems:Array<DisplayObject>):Bool {
		...
		var leftAnchorDisplayObject:DisplayObject = layoutData.leftAnchorDisplayObject;
		if (leftAnchorDisplayObject != null
			&& (items.indexOf(leftAnchorDisplayObject, nextIndex) >= nextIndex
				|| unpositionedItems.indexOf(leftAnchorDisplayObject) >= 0)) {
			return false;
		}
		var rightAnchorDisplayObject:DisplayObject = layoutData.rightAnchorDisplayObject;
		if (rightAnchorDisplayObject != null
			&& (items.indexOf(rightAnchorDisplayObject, nextIndex) >= nextIndex
				|| unpositionedItems.indexOf(rightAnchorDisplayObject) >= 0)) {
			return false;
		}
		var topAnchorDisplayObject:DisplayObject = layoutData.topAnchorDisplayObject;
		if (topAnchorDisplayObject != null
			&& (items.indexOf(topAnchorDisplayObject, nextIndex) >= nextIndex || unpositionedItems.indexOf(topAnchorDisplayObject) >= 0)) {
			return false;
		}
		var bottomAnchorDisplayObject:DisplayObject = layoutData.bottomAnchorDisplayObject;
		if (bottomAnchorDisplayObject != null
			&& (items.indexOf(bottomAnchorDisplayObject, nextIndex) >= nextIndex
				|| unpositionedItems.indexOf(bottomAnchorDisplayObject) >= 0)) {
			return false;
		}
		var horizontalCenterAnchorDisplayObject:DisplayObject = layoutData.horizontalCenterAnchorDisplayObject;
		if (horizontalCenterAnchorDisplayObject != null
			&& (items.indexOf(horizontalCenterAnchorDisplayObject, nextIndex) >= nextIndex
				|| unpositionedItems.indexOf(horizontalCenterAnchorDisplayObject) >= 0)) {
			return false;
		}
		var verticalCenterAnchorDisplayObject:DisplayObject = layoutData.verticalCenterAnchorDisplayObject;
		if (verticalCenterAnchorDisplayObject != null
			&& (items.indexOf(verticalCenterAnchorDisplayObject, nextIndex) >= nextIndex
				|| unpositionedItems.indexOf(verticalCenterAnchorDisplayObject) >= 0)) {
			return false;
		}
		return true;
	}
	...
	private function isReferenced(item:DisplayObject, items:Array<DisplayObject>):Bool {
		...
		for (i in 0...itemCount) {
			...
			if (layoutData.leftAnchorDisplayObject == item
				|| layoutData.horizontalCenterAnchorDisplayObject == item
				|| layoutData.rightAnchorDisplayObject == item
				|| layoutData.topAnchorDisplayObject == item
				|| layoutData.verticalCenterAnchorDisplayObject == item
				|| layoutData.bottomAnchorDisplayObject == item) {
				return true;
			}
		}
		return false;
	 ...
	 * @private
	 */
	private function validateItems(items:Array<DisplayObject>, explicitWidth:Float, explicitHeight:Float, maxWidth:Float, maxHeight:Float, force:Bool):Void {
		var needsWidth:Bool = explicitWidth != explicitWidth; // isNaN
		var needsHeight:Bool = explicitHeight != explicitHeight; // isNaN
		...
		for (i in 0...itemCount) {
			...
			if (item is ILayoutDisplayObject) {
				...
				if (layoutData != null) {
					...
					var measureItem:IMeasureDisplayObject = cast(item, IMeasureDisplayObject);
					if (needsWidth) {
